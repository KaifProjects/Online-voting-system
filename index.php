    ...
    <div class="container h-100">
        <div class="d-flex justify-content-center h-100">
            <div class="user_card">
                <div class="d-flex justify-content-center">
                    <div class="brand_logo_container">
                        <img src="assets/images/candidate_photos/logo.gif" class="brand_logo" alt="Logo">
                    </div>
                </div>
			...
			?>
				...
				{ 
					?>
                    <div class="bg-white text-success text-center my-3">Your account has been created successfully!</div>
                <?php 
			} elseif (isset($_GET['invalid'])) { 
				?>
                    <div class="bg-white text-danger text-center my-3">Password mismatched, please try again.</div>
                <?php 
			}elseif (isset($_GET['not_registered'])) { 
				?>
                    <div class="bg-white text-warning text-center my-3">Sorry,you are not registered!</div>
                <?php 
			} elseif (isset($_GET['invalid_access'])) { 
				?>
                    <div class="bg-white text-danger text-center my-3">Invalid Username or Password!</div>
                <?php 
			} elseif (isset($_GET['contact_exists'])) { 
				?>
                    <div class="bg-white text-warning text-center my-3">This contact number is already registered!</div>
                <?php 
			}
			?>
...
if (isset($_POST['sign_up_btn'])) {
    ...
    if ($su_password == $su_retype_password) {
        ...
        if (mysqli_num_rows($check_query) > 0) {
            // Contact number already exists, inform the user
            echo "<script>window.location.href='index.php?sign-up=1&contact_exists=1';</script>";
            exit();
        } else {
            // Insert Query
            ...
            echo "<script>window.location.href='index.php?sign-up=1&registered=1';</script>";
            exit();
        }
    } else {
        echo "<script>window.location.href='index.php?sign-up=1&invalid=1';</script>";
        exit();
...

{
    ...
    <?php
		...
		echo "<script>window.location.href='index.php?invalid_access=1';</script>";
	}
		
	}else{
		echo "<script>window.location.href='index.php?sign-up=1&not_registered=1';</script>";
